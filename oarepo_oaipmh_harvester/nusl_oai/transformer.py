...
def parse_place(place):
    ...
    if place:
        res["place"] = place
        res["country"] = next((item for item in data if item["id"] == country), None)
    return res
...
def transform_720_contributor(md, rec, value):
    if value[0]:
        data = reader('contributorType')
        md.setdefault('contributors', []).append(
            make_dict(
                'fullName', value[0],
                'nameType', 'Personal',
                'role', next((item for item in data if item["id"] == value[1]), None)
            ))
        print('')
...
def transform_980_resource_type(md, rec, value):
    ...
    data = reader('resourceType')

    md["resourceType"] = {
        "bakalarske_prace": next((item for item in data if item["title"]["cs"] == "Bakalářská práce"), None),
        "diplomove_prace": next((item for item in data if item["title"]["cs"] == "Diplomová práce"), None),
        "disertacni_prace": next((item for item in data if item["title"]["cs"] == "Rigorózní práce"), None),
        "rigorozni_prace": next((item for item in data if item["title"]["cs"] == "Disertační práce"), None),
        "habilitacni_prace": next((item for item in data if item["title"]["cs"] == "Habilitační práce"), None)
    }.get(value, value)


def reader(type):
    try:
        ResourceType =  os.path.join(os.path.dirname(__file__), '../data/' + type + '.xlsx')
        rdr = ExcelReader(
            vocabulary_type=type, origin=ResourceType)
        return list(rdr.read())
    except Exception as e:
        print(e)

@matches('996__a', '996__b', '996__9', paired=True)
def transform_996_accessibility(md, rec, value):
    data = reader('accessRights')
    md['accessibility'] = make_array(
        value[0], {'lang': 'cs', 'value': value[0]},
        value[1], {'lang': 'en', 'value': value[1]},
    )
    if value[0]:
        md['accessRights'] = get_access_rights(data, text=value[0])
    else:
        md['accessRights'] = get_access_rights(data, slug=value[2] or "c_abf2")
...
def get_access_rights(data, text=None, slug=None):
    if not slug:
        sentence_dict = {
    ...
    return {
        'c_abf2':  next((item for item in data if item["id"] == "c_abf2"), None),
        'c_f1cf':  next((item for item in data if item["id"] == 'c_f1cf'), None),
        'c_16ec':  next((item for item in data if item["id"] == 'c_16ec'), None),
        'c_14cb':  next((item for item in data if item["id"] == 'c_14cb'), None),
    }.get(slug, slug)
...
def transform_04107_language(md, rec, value):
    # TODO: taxonomy
    data = reader('languages')
    try:
        # md.setdefault('languages', []).append(
        #     get_alpha2_lang(value))
        md.setdefault('languages', []).append(next((item for item in data if item["id"] == get_alpha2_lang(value)), None))
...
def transform_336_certifikovana_metodika(md, rec, value):
    data = reader('resourceType')
    md["resourceType"] = next((item for item in data if item["id"] == 'certified-methodologies'), None)
...
def parse_rights(text):
    ...
    data = reader('licenses')
    if not license:
        return
    return next((item for item in data if item["id"] == license), None)
